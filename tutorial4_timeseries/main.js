...
function init() {
  ...
  yScale = d3
    .scaleLinear()
    .domain([0, d3.max(state.data, d => d.fertility_rate)])
...
function draw() {
  ...
  yScale.domain([0, d3.max(filteredData, d => d.fertility_rate)]);
    ...
    .call(
      selection =>
        selection
          ...
          .attr("cy", d => yScale(d.fertility_rate)) // started from the bottom, now we're here
    );
